...
const envatoCookies =
  "__cf_bm=BWzWWF5kGGQ3X6Mw0tyz3uP.cl5irFbcsUq2iCiz11M-1744121368-1.0.1.1-.NYaWae5V370Cb3HLFgyavuxPooJKOSR4jvpjnmQeKyKJnQYHScQE29xCM6Sdeo9UkRQgp8QrzcPGXkwohgpb54uloBq7t_ms1GTQ27ZIgc";
...
app.post("/video-previewV2", async (req, res) => {
  try {
    ...
    if (!urls || !Array.isArray(urls) || urls.length === 0 || !duration) {
      return res
        .status(400)
        .json({ error: "urls (as an array) and duration are required." });
    }
    ...
    const ratioParam =
      firstUrlObj.searchParams.get("ratio")?.toLowerCase() || "v";
    ...
    console.log(
      `Target resolution set to: ${targetWidth}x${targetHeight} (${
        ratioParam === "h" ? "Horizontal" : "Vertical"
      })`
    );
    ...
    for (const url of urls) {
      ...
      try {
        ...
        if (isYouTube) {
          ...
          if (!videoId) {
            console.log(
              `Skipping URL ${url} because video id could not be determined.`
            );
            continue;
          }
          ...
          if (!fs.existsSync(mediaPath)) {
            if (ongoingDownloads.has(videoId)) {
              console.log(
                `Video ${videoId} is already downloading. Waiting for download to complete...`
              );
              ...
              const downloadPromise = runCommand(
                `yt-dlp -f "bestvideo[ext=mp4]+bestaudio=none/bestvideo[ext=mp4]" -o "${mediaPath}" "${downloadUrl}"`
              );
          ...
          if (!fs.existsSync(mediaPath)) {
            if (ongoingDownloads.has(hash)) {
              console.log(
                `Direct video for ${url} is already downloading. Waiting for download to complete...`
              );
          ...
          if (!fs.existsSync(imagePath)) {
            if (ongoingDownloads.has(hash)) {
              console.log(
                `Image for ${url} is already downloading. Waiting for download to complete...`
              );
              await ongoingDownloads.get(hash);
            } else {
          ...
          console.log(
            `🎥 Creating video from image ${imagePath} with smooth zoom-out for ${clipDuration}s...`
          );
        ...
        console.log(
          `🎥 Extracting clip from ${mediaPath} starting at ${startTime}s (duration ${clipDuration}s)....`
        );
        await runCommand(
          `ffmpeg -y -ss ${startTime} -i "${mediaPath}" -t ${clipDuration} -c:v libx264 -crf 23 -preset fast "${clipFile}"`
        );
    ...
    if (clipFiles.length === 1) {
      console.log(
        "Only one clip available, scaling it to the target resolution..."
      );
      await runCommand(
      ...
      if (clipFiles.length === 0) {
        ...
        return res
          .status(400)
          .json({ error: "No clips were created. Check your input URLs." });
      }
      ...
      const mergeCommand =
        `ffmpeg -y ${inputsCommand} -filter_complex "${filterComplex}" -map "[out]" ` +
      ...
      await runCommand(
        `ffmpeg -y -i "${mergedVideoPath}" -an -c copy "${finalVideoPath}"`
      );
      ...
      const finalPublicVideoPath = path.join(
        publicDir,
        `final_video_${uniqueId}.mp4`
      );
      ...
      const fileUrl = `${req.protocol}://${req.get(
        "host"
      )}/videos/${path.basename(finalPublicVideoPath)}`;
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          error: "youtubeUrls (as an array) and duration are required.",
        });
    }
    ...
    if (validUrls.length === 0) {
      return res
        .status(400)
        .json({
          error: "No valid YouTube URLs with a 't' query parameter provided.",
        });
    }
    ...
    for (const url of validUrls) {
      ...
      if (!fs.existsSync(videoPath)) {
        if (ongoingDownloads.has(videoId)) {
          ...
          const downloadPromise = runCommand(
            `yt-dlp -f "bestvideo[ext=mp4]+bestaudio=none/bestvideo[ext=mp4]" -o "${videoPath}" "${downloadUrl}"`
          );
          ongoingDownloads.set(videoId, downloadPromise);
          try {
            await downloadPromise;
      ...
      const clipFile = path.join(tempDir, `clip_${clipIndex + 1}.mp4`);
      console.log(
        `🎥 Extracting clip from ${videoPath} starting at ${startTime}s (duration ${clipDuration}s)...`
      );
      // Using -c copy since we don't want to modify the original resolution
      await runCommand(
        `ffmpeg -ss ${startTime} -i "${videoPath}" -t ${clipDuration} -c copy "${clipFile}"`
      );
      clipFiles.push(clipFile);
      clipIndex++;
    }
    ...
    if (clipFiles.length === 0) {
      return res
        ...
        .json({
          error:
            "No clips could be extracted. Check the input URLs and their 't' parameters.",
        });
    }
    ...
    await runCommand(
      `ffmpeg -f concat -safe 0 -i "${concatFilePath}" -c copy "${mergedVideoPath}"`
    );
    ...
    console.log("🔇 Removing audio from the final video...");
    await runCommand(
      `ffmpeg -i "${mergedVideoPath}" -an -c copy "${finalVideoPath}"`
    );
    console.log("✅ Audio removed successfully!");
